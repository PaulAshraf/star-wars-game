...
void myDisplay(void)
{
	...
	if (winn) {
		displayText("You have won the game press spacebar to end", At.x - 12, At.y + 6);
	}
	if (losee) {
		displayText("You have lost the game press spacebar to end", At.x - 12, At.y + 6);
	}
...
void lose() {
	losee = true;
	displayText("You have lost the game press spacebar to end", At.x - 12, At.y + 6);
	PlaySound(TEXT("losttheme.wav"), NULL, SND_ASYNC | SND_FILENAME);
	glutPostRedisplay();
}
